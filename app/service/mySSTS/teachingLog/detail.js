// app/service/mySSTS/teachingLog/detail.js
'use strict';
 ...
 */
class TeachingLogDetailService extends Service {
  async getTeachingLogDetail({ JSESSIONID_A, teachingClassId, token }) {
    // 生成 8 位 16 进制随机字符串
    const randomHex = Array.from({ length: 8 }, () =>
      Math.floor(Math.random() * 16).toString(16)
    ).join('');
    // 生成 0 到 1 之间的随机浮点数frame/component/pagegrid/pagegrid.action?frameControlSubm字符串，并截取小数点后 13 位
    ...
    const plainTextData = {
      ...
      pageSize: 100,
      group: [],
      aggregate: [{ field: 'LESSON_HOURS', aggregate: 'sum' }],
...
module.exports = TeachingLogDetailService;
